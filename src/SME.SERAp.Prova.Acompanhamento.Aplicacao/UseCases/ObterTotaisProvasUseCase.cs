...
namespace SME.SERAp.Prova.Acompanhamento.Aplicacao.UseCases
{
    ...
    public class ObterTotaisProvasUseCase : AbstractUseCase, IObterTotaisProvasUseCase
    {
        ...
        public async Task<List<TotalDto>> Executar(FiltroDto filtro)
        {
            ...
            var provas = await mediator.Send(new ObterProvasQuery(filtro.AnoLetivo, filtro.ProvaSituacao));
            ...
            provasAluno = provasAluno.Where(x => provas.Any(p => p.Id == x.ProvaId.ToString())).ToList();

            if (provasAluno == null || !provasAluno.Any())
                return listaTotais;

            var totalProvas = provasAluno.ToList().Count;
            var retornoTotalAluno = new TotalDto("Total de Provas Iniciadas no Dia.", "#1B80D4", totalProvas.ToString());
